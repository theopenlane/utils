...
func TestEncrypt(t *testing.T) {
	...
	for i, scenario := range scenarios {
		result, err := keygen.Encrypt([]byte(scenario.data), scenario.key)
		...
		// try to decrypt
		if result != "" {
			decrypted, _ := keygen.Decrypt(result, scenario.key)
...
func TestDecrypt(t *testing.T) {
	...
	for i, scenario := range scenarios {
		result, err := keygen.Decrypt(scenario.cipher, scenario.key)
